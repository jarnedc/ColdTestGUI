...
bool dbHandler::checkHybrid(string hybridID){
	...
	try{
		...
		if (res->rowsCount()==0){
			cout << "[db_handler::checkHybrid] Hybrid not tested. HybridID: "<< hybridID << endl;
			return true;
		}
		else{
			cout << "[db_handler::checkHybrid] Hybrid tested. HybridID: " << hybridID << endl;
			return false;
		}
	}
	catch(sql::SQLException &e){
		cout << "[db_handler::checkHybrid] Problem with the database. Try to reconnect." << endl;
...
bool dbHandler::insertNewTestResult(string hybridID, string calibrationStatus, string pass_test, string nCBCs, string timestamp){
		cout << "INSERT INTO FEH_test_results (date,time,hybrid_type,hybrid_serial_number,passed_test,exit_code,test_result) VALUES (FROM_UNIXTIME("+timestamp+",  '%D.%M.%Y'), FROM_UNIXTIME("+timestamp+", '%h:%i:%s')," << nCBCs << ", " << hybridID << ", 1," << calibrationStatus << ");" << endl;
	stmt = con->createStatement();
	try {
		...
		cout << "INSERT crash, checking status of the last input..." << endl;
		if(checkHybrid(hybridID)) {
			cout << "[db_handler::checkHybrid] Problem with inserting into the DB." << endl;
			return false;
		}
		else { 
			cout << "[db_handler::checkHybrid] Insert successfull." << endl;
			return true;
		}
	}
	return true;
